...
export default function EndEventBox({
  ...
  return (
    <Modal
      ...
      open={active} 
      onClose={clickExit}
      xButtonTabIndex={planetCount===0 ? -1 : tabable(2, !isTimeoutPopupActive)}
      bgUrl={loader.get(MAIN_CONGRATS).src}
    >
      {type === END_EVENT_TYPES.STABLE ? (
        <>
          <AudioDescription
            description={`${planetCount===0 ? contentMatrix.main.exitEvent.audioDescription : contentMatrix.main.endEvents.audioDescription} ${
              planetCount===0 ? '' : contentMatrix.main.endEvents.stable.title
            } ${stableDescription(planetCount)} ${contentMatrix.main.exitEvent.adTabAriaInstructionEndEvent}`}
            autoFocus={true}
          />
          <Title>{planetCount===0 ? '' : contentMatrix.main.endEvents.stable.title}</Title>
        ...
        <>
          <AudioDescription
            description={`${planetCount===0 ? contentMatrix.main.exitEvent.audioDescription : contentMatrix.main.endEvents.audioDescription} ${
              planetCount===0 ? '' : contentMatrix.main.endEvents.relativelyStable.title
            } ${semiStableDescription(planetCount)} ${contentMatrix.main.exitEvent.adTabAriaInstructionEndEvent}`}
            autoFocus={true}
          />
          <Title>{planetCount===0 ? '' : contentMatrix.main.endEvents.relativelyStable.title}</Title>
        ...
        <>
          <AudioDescription
            ...
            } ${crashesDescription(crashCount)} ${contentMatrix.main.exitEvent.adTabAriaInstructionEndEvent}`}
            autoFocus={true}
          />
      ...
      <GotItButton
        aria-roledescription={planetCount===0 ? 'Press, the center key to return to the game' :
      ...
      >
        {planetCount===0 ? 'Got it': contentMatrix.main.endEvents.endButtonLabel}
      </GotItButton>
    </Modal>
  );
}
...
const stableDescription = (planetCount) => {
  return (
    planetCount===0 ? 'You have no planets in orbit! Place planets around the star and see what happens!' 
...
const semiStableDescription = (planetCount) => {
  return (
    ...
    relatively stable ${planetCount===1 ? 'orbit' : 'orbits'}! That is not an easy feat.`);
};
...
const GotItButton = styled(Button)`
  ...
  :active {
    background: ${({ theme }) =>
    theme.color.buttonPressedBackground};
  } */
`;
