...
export default function Credit({ active, handleClick, tabIndex }) {
  ...
  return (
    ...
    aria-label={active ? 'A list of credits appears in the center of the screen. Tab once with the right arrow key for the list, or tab twice for the close button to return to the keypad introduction screen' : ''}
    ...
    <CreditsAD
      ...
      aria-roledescription={`List of Credits, There are ${contentMatrix.credits.length} entries on the screen. 
      Tab once to the right to skip the credits. Credits.
       ${(contentMatrix.creditsAD ? contentMatrix.creditsAD : contentMatrix.credits).map((credit, index)=>(
        `${credit} , `
      ))}`}
    />
    <Title>
    ...
    >
      ...
      >
        ...
        ></ReactSlider>
           ...
           <Scrollbar 
              style={{  bottom:0, left: 20, width: 640, height: ScrollContentHeight, }}
    ...
    <CloseButton   
        ...
        ref={buttonRef}
        aria-roledescription={'Close, button. Press the center key to return to the keypad introduction screen'}
      />
...
const Slider = styled.div`
  /* background: ${props=>props.isFocused ? ({ theme }) => theme.color.focus : ''}; */
