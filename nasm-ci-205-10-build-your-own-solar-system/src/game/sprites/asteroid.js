...
export const createAsteroid = (scene, config) => {
  ...
  const dataWithPreloadedImages = {...data, image: loader.get(data.image).src, thumb: loader.get(data.thumb).src};
...
export const createAsteroidBelt = (scene, config) => {
    ...
    for (var i = 0; i < dotsPerCircle; i++) {
      ...
      var desiredRadianAngleOnCircle = interval * i;
        ...
        const asteroid = createAsteroid(scene, {
          ...config,
          x: sunX + (distance+distanceNoise) * (Math.cos(desiredRadianAngleOnCircle*angleNoise)),
          y: sunY + (distance+distanceNoise) * (Math.sin(desiredRadianAngleOnCircle*angleNoise)),
        });
  ...
  const asteroidBeltGroup = new AsteroidGroup(scene, asteroids, { distance: distance });
